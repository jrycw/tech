...
### marimo
::: {.callout-note}
# Interactivity Temporarily Unavailable
It looks like the `quarto-marimo` plugin is currently experiencing some issues, so this post isn’t interactive for now. I’ll update it to be interactive once the issue is resolved.

In the meantime, you can view the interactive version on marimo.app via [this link](https://marimo.app/l/de67qq).
::: 

Our goal is to build an object called `WigGT` that behaves like a `GT` object from Great Tables. As we build the table, we maintain two versions:

* the original (non-interactive) table, and
* an interactive version enhanced with a marimo widget (a slider, in this case).

The result is an easy way to move back and forth through the stages of table construction using Great Tables.
...
#### `get_allowed_member_names()`
To begin, we need a way to retrieve all attributes and functions provided by Great Tables. For now, I manually constructed the list, but it would be great if Great Tables exposed this information for easier third-party integration.
...
#### `lazify` decorator
`lazify` is a decorator that records each method call on `WigGT` (as long as the method exists in Great Tables). These calls are stored and deferred until we're ready to "collect" and render the full table.
...
1. We keep the original (non-interactive) tables in `self._tables`, and provide a public `.tables` API to retrieve them.
2. Interactive tables (with widgets) are stored in `self._wtables`, but no public API is provided. Interaction is intended to occur via marimo's slider widget.
3. The HTML rendering logic follows [marimo's display conventions](https://docs.marimo.io/guides/integrating_with_marimo/displaying_objects/), preferring `_display_`, then `_repr_html_`, and finally `_mime_`.
...
### How to use
Now we're ready to time travel!

First, we create a slider to represent each step in the table-building process. Step 0 is the original `GT` object; step 1 applies the first function, and so on.
...
Next, we use [an example from the Great Tables documentation](https://posit-dev.github.io/great-tables/examples/) to build a total of seven tables: one original, and six more via chained method calls.
```{python}
#| code-fold: false
#| code-summary: Show code
lazy_wig_gt = (
...
lazy_wig_gt
```
At first, you’ll see a slider widget embedded in the table’s source note, but it won’t work yet.

We need to finalize the process by calling `.collect()`, which triggers execution of the stored method calls. Once `.collect()` is called, the slider becomes functional — try dragging it to observe the table at each stage of construction!
```{python}
#| code-fold: false
#| code-summary: Show code
wig_gt = lazy_wig_gt.collect()
...
Lastly, you can retrieve all built tables using the `.tables` property.
```{python}
#| code-fold: false
#| code-summary: Show code
#| output: false
