...
7. The original table featured two rings, but I was only able to implement the inner ring.
8. While the color codes and font properties could be further enhanced, Great Tables successfully replicates the table's structure and style with only minor differences.

![](reciprocal_tariffs.png){fig-align="center" fig-alt="Cloned Reciprocal Tariffs Table"}
...
logo_url = "https://upload.wikimedia.org/wikipedia/commons/thumb/3/36/Seal_of_the_President_of_the_United_States.svg/800px-Seal_of_the_President_of_the_United_States.svg.png"
logo = vals.fmt_image(logo_url, height=150)[0]
...
def change_border_radius(
    ...
    <div style="background-color: {background_color1};border: None">\
        <div style="border-radius: {border_radius};background-color:{background_color2};">&nbsp;{x}</div>\
    </div>\
    """
...
(
    ...
    .cols_label(
        {
            "country": html(
                f'<br><div style="text-align: left;">{logo} <span style="color: {gold}; font-size: 40px;">&nbsp&nbsp;Reciprocal Tariffs</span></div><br><b>Country</b>'
            ),
            "tariffs_charged": html(
                '<b>Tariffs Charged<br>to the U.S.A.</b><br><span style="font-size: 12px;">Including<br>Currency Manipulation<br>and Trade Barrierst</span>'
            ),
            "reciprocal_tariffs": html(
                "<b>U.S.A. Discountedd<br>Reciprocal Tariffs</b>"
            ),
            "0": "",
            "1": "",
            "2": "",
    ...
    # .save("gt.png", web_driver="firefox", window_size=(1200, 1000))
)
```
