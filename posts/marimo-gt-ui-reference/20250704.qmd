...
One interesting discovery: when a widget's value is updated via an `on_change` callback—such as with the **run button**—the new state sometimes doesn't persist as expected. This might be addressed by better leveraging [reactive state](https://docs.marimo.io/guides/state/?h=state) in marimo, which I plan to explore further.
